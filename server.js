...
/** Ensure board object has valid shape (mutates). Empty columns break Kanban + recurring tasks. */
function normalizeBoard(board) {
  ...
  if (typeof board.employeeProfiles !== 'object' || board.employeeProfiles === null) board.employeeProfiles = {};
  if (typeof board.employeeAccuracyHistory !== 'object' || board.employeeAccuracyHistory === null) {
    board.employeeAccuracyHistory = {};
  }
}

function ensureBoardFileMigrated() {
  let board = readBoard();
  const hadFile = !!board;
  if (!board) board = {};
  const before = JSON.stringify(board.columns);
  normalizeBoard(board);
  const after = JSON.stringify(board.columns);
  if (!hadFile || before !== after) {
    writeBoard(board);
  }
}

ensureBoardFileMigrated();
...
// Same contract as Vercel api/store-status (client hides Redis warning when backend !== 'file').
app.get('/api/store-status', (req, res) => {
  const m = readManager();
  const employees = readEmployees();
  res.json({
    backend: 'filesystem',
    hasManager: !!(m && m.email),
